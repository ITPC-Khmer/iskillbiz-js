<template>
  <div class="permissions-page">
        ...
        <div class="d-flex justify-content-between align-items-start mb-3">
          <div class="flex-grow-1">
            ...
            <p class="text-muted small mb-0">
              Super_admin only — Create and manage permissions to define what
              users can do
            </p>
          </div>
          ...
          <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
              <tr v-for="p in permissions" :key="p.id">
                ...
                <td>{{ p.description }}</td>
                <td>
                  <button class="btn btn-link btn-sm p-0" @click="select(p)">
                    Edit
                  </button>
                </td>
              </tr>
    ...
    <RightDrawer
      ref="drawer"
      :title="form.id ? 'Update Permission' : 'Create Permission'"
    >
...
async function submit() {
  ...
  try {
    if (form.id) {
      const res = await api.put(`/permissions/${form.id}`, form, {
        token: auth.token.value,
      });
      permissions.value = permissions.value.map((p) =>
        p.id === form.id ? res.data.permission : p
      );
      msg.value = "Updated";
    } else {
