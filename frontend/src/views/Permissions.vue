<template>
  <div class="card shadow-sm mb-3">
    <div class="card-body">
      <div class="d-flex justify-content-between align-items-center mb-2">
        <h3 class="card-title mb-0">Permissions (super_admin)</h3>
        ...
        <table class="table table-sm align-middle">
          ...
          <tbody>
            <tr v-for="p in permissions" :key="p.id">
              <td>{{ p.name }}</td>
              <td>{{ p.description }}</td>
              <td><button class="btn btn-link btn-sm" @click="select(p)">Edit</button></td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  <div class="card shadow-sm">
    <div class="card-body">
      <h4 class="card-title">{{ form.id ? 'Update Permission' : 'Create Permission' }}</h4>
